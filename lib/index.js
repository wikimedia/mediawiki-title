...
Normalizer.prototype._getSiteInfo = function(domain) {
    ...
    if (self._siteInfo[domain]) {
        ...
        .then(function(res) {
            ...
            self._siteInfo[domain].invalidTitleRegex =
                self._createInvalidTitleRegex(res.general.legaltitlechars);
            self._siteInfo[domain].namespaces = res.namespaces;
...
Normalizer.prototype._getNSIndex = function(nsName, siteInfo) {
    return Object.keys(siteInfo.namespaces).find(function(nsId) {
        var ns = siteInfo.namespaces[nsId];
        return ns.canonical === nsName || ns['*'] === nsName;
    });
};
