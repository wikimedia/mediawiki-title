...
function _normalizeSpecialPage(title, siteInfo) {
    var parts = title.split('/');
    var first = parts[0];
    var alias = arrayFind(siteInfo.specialpagealiases || [], function(o) {
        return first === o.realname || o.aliases.indexOf(first) > -1;
    });
    if (alias) {
...
Title.newFromText = function(title, siteInfo, defaultNs) {
    ...
    if (result.namespace.isSpecial()) {
        result.title = _normalizeSpecialPage(result.title, siteInfo);
    }
